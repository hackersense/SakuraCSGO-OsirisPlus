...
static void applyKnife(const inventory_changer::backend::BackendSimulator& backend, CSPlayerInventory& localInventory, Entity* local) noexcept
{
    ...
    for (auto& weapons = local->weapons(); auto weaponHandle : weapons) {
        ...
        if (definitionIndex != WeaponId::Knife && definitionIndex != WeaponId::KnifeT)
            continue;
...
static void applyWeapons(CSPlayerInventory& localInventory, Entity* local) noexcept
{
    ...
    for (int i = memory->globalVars->maxClients + 1; i <= highestEntityIndex; ++i) {
        ...
        const auto weaponTeam = getWeaponTeam(definitionIndex);
        const auto team = (local != localPlayer.get() || weaponTeam == 0) ? localTeam : (weaponTeam == 1 ? csgo::Team::TT : csgo::Team::CT);
        const auto loadoutSlot = def->getLoadoutSlot(team);
        const auto optionalItem = getItemFromLoadout(inventory_changer::InventoryChanger::instance().getBackend().getLoadout(), team, loadoutSlot);
        if (!optionalItem.has_value())
            continue;
...
static void applyPlayerAgent() noexcept
{
    ...
    if ((gameMode->getInt() == 0 && gameType->getInt() == 6) || (gameMode->getInt() == 1 && gameType->getInt() == 4))
        return;
...
{
    if (ImGui::MenuItem(Language::getText(LanguageID::GUI_TAB_INVENTORYCHANGER))) {
        windowOpen = true;
        ImGui::SetWindowFocus(Language::getText(LanguageID::GUI_TAB_INVENTORYCHANGER));
        ImGui::SetWindowPos(Language::getText(LanguageID::GUI_TAB_INVENTORYCHANGER), { 100.0f, 100.0f });
    }
}

void InventoryChanger::tabItem() noexcept
{
    if (ImGui::BeginTabItem(Language::getText(LanguageID::GUI_TAB_INVENTORYCHANGER))) {
...
void InventoryChanger::drawGUI(bool contentOnly)
{
    if (!contentOnly) {
        ...
        if (!ImGui::Begin(Language::getText(LanguageID::GUI_TAB_INVENTORYCHANGER), &windowOpen, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse)) {
            ImGui::End();
            return;
        }
    }
...
void InventoryChanger::playerSwap(GameEvent& event)
{
    if (!localPlayer)
        return;

    if (event.getInt("isbot") || event.getInt("userid") != localPlayer->getUserId())
        return;

    needUpdate = true;
}

void InventoryChanger::playerSpawn(GameEvent& event)
{
    if (!localPlayer)
        return;
