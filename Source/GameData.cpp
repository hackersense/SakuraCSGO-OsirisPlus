...
static UtlVector<ActiveSoundInfo> activeSounds;
...
void GameData::update() noexcept
{
    ...
    interfaces->sound->getActiveSounds(activeSounds);

    for (auto& sound : activeSounds)
    {
        if (sound.soundSource < 1 || sound.soundSource > 64 || !sound.origin->notNull())
            continue;
        ...
        if (const auto entity = interfaces->entityList->getEntity(sound.soundSource); entity && entity->isPlayer())
            if (auto player = playerByHandleWritable(entity->handle()); player)
            {
                ...
                player->audible = true;
                player->dormantOrigin = *sound.origin;
            }
    }
    ...
    {
        Channel channel = memory->channels[memory->activeChannels->list[i]];
        if (channel.soundSource < 1 || channel.soundSource > 64 || !channel.origin.notNull())
            continue;

        if (const auto entity = interfaces->entityList->getEntity(channel.soundSource); entity && entity->isPlayer())
            if (auto player = playerByHandleWritable(entity->handle()); player)
            {
                player->becameDormant = memory->globalVars->realtime;
                player->audible = true;
                player->dormantOrigin = channel.origin;
            }
    }
    */
...
void LocalPlayerData::update() noexcept
{
    ...
    if (const auto activeWeapon = localPlayer->getActiveWeapon()) {
        ...
        shootingAndPistol = localPlayer->shotsFired() > 1;
        shooting = activeWeapon->isPistol() ? !inReload && nextWeaponAttack > memory->globalVars->serverTime() : shootingAndPistol;
