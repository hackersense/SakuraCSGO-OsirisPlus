...
void GUI::renderMenuBar() noexcept
{
    if (ImGui::BeginMainMenuBar()) {
        menuBarItem(Language::getText(LanguageID::GUI_TAB_AIMBOT), window.aimbot);
        AntiAim::menuBarItem();
        menuBarItem(Language::getText(LanguageID::GUI_TAB_TRIGGERBOT), window.triggerbot);
        Backtrack::menuBarItem();
        Glow::menuBarItem();
        menuBarItem(Language::getText(LanguageID::GUI_TAB_CHAMS), window.chams);
        StreamProofESP::menuBarItem();
        Visuals::menuBarItem();
        ProfileChanger::menuBarItem();
        InventoryChanger::menuBarItem();
        Sound::menuBarItem();
        menuBarItem(Language::getText(LanguageID::GUI_TAB_STYLE), window.style);
        Misc::menuBarItem();
        menuBarItem(Language::getText(LanguageID::GUI_TAB_CONFIG), window.config);
...
void GUI::renderAimbotWindow(bool contentOnly) noexcept
{
    if (!contentOnly) {
        ...
        ImGui::Begin(Language::getText(LanguageID::GUI_TAB_AIMBOT), &window.aimbot, windowFlags);
    }
...
void GUI::renderTriggerbotWindow(bool contentOnly) noexcept
{
    if (!contentOnly) {
        ...
        ImGui::Begin(Language::getText(LanguageID::GUI_TAB_TRIGGERBOT), &window.triggerbot, windowFlags);
    }
    static int currentCategory{ 0 };
    ImGui::PushItemWidth(110.0f);
    ImGui::PushID(0);
...
void GUI::renderChamsWindow(bool contentOnly) noexcept
{
    if (!contentOnly) {
        if (!window.chams)
            return;
        ImGui::SetNextWindowSize({ 0.0f, 0.0f });
        ImGui::Begin(Language::getText(LanguageID::GUI_TAB_CHAMS), &window.chams, windowFlags);
    }
...
void GUI::renderStyleWindow(bool contentOnly) noexcept
{
    if (!contentOnly) {
        if (!window.style)
            return;
        ImGui::SetNextWindowSize({ 0.0f, 0.0f });
        ImGui::Begin(Language::getText(LanguageID::GUI_TAB_STYLE), &window.style, windowFlags);
    }
...
void GUI::renderConfigWindow(bool contentOnly) noexcept
{
    if (!contentOnly) {
        ...
        if (!ImGui::Begin(Language::getText(LanguageID::GUI_TAB_CONFIG), &window.config, windowFlags)) {
            ImGui::End();
            return;
        }
    }
