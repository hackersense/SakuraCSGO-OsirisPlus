...
{
    BoundingBox bbox{ playerData, config.box.scale };

    float distanceToLocal = playerData.distanceToLocal;
    ...
    FontPush font{ config.font.name, distanceToLocal };

    if (config.name.enabled) {
        const auto nameSize = renderText(distanceToLocal, config.textCullDistance, config.name.asColor4(), playerData.name.c_str(), { (bbox.min.x + bbox.max.x) / 2, bbox.min.y - 2 });
    ...
    if (config.weapon.enabled && !playerData.activeWeapon.empty()) {
        const auto weaponTextSize = renderText(distanceToLocal, config.textCullDistance, config.weapon.asColor4(), playerData.activeWeapon.c_str(), { (bbox.min.x + bbox.max.x) / 2, bbox.max.y + 1 }, true, false);
...
static bool renderPlayerEsp(PlayerData& playerData, const Player& playerConfig, const bool allowDormant = false) noexcept
{
    ...
    if (allowDormant && playerData.dormant && playerData.dormantOrigin.notNull())
    {
        playerData.coordinateFrame.setOrigin(playerData.dormantOrigin);
        playerData.origin = playerData.dormantOrigin;

        const auto entity = interfaces->entityList->getEntityFromHandle(playerData.handle);
...
{
    auto allWeapons = config->streamProofESP.weapons["All"];
    const auto& config = itemConfig.enabled ? itemConfig : (parentConfig.enabled ? parentConfig : allWeapons);
    if (config.enabled)
...
void StreamProofESP::render() noexcept
{
    ...
    for (const auto& player : GameData::players()) {
        ...
        {
            renderPlayerEsp((PlayerData&)player, playerConfig["Dormant"]);
            continue;
        }

        if (player.dormant && player.fadingAlpha() == 0.0f)
            continue;

        if (!renderPlayerEsp((PlayerData&)player, playerConfig["All"]))
            renderPlayerEsp((PlayerData&)player, playerConfig[player.visible ? "Visible" : "Occluded"]);
    }
}
...
{
    if (ImGui::MenuItem(Language::getText(LanguageID::GUI_TAB_ESP))) {
        windowOpen = true;
        ImGui::SetWindowFocus(Language::getText(LanguageID::GUI_TAB_ESP));
        ImGui::SetWindowPos(Language::getText(LanguageID::GUI_TAB_ESP), { 100.0f, 100.0f });
    }
}

void StreamProofESP::tabItem() noexcept
{
    if (ImGui::BeginTabItem(Language::getText(LanguageID::GUI_TAB_ESP))) {
        drawGUI(true);
        ImGui::EndTabItem();
    }
}
...
void StreamProofESP::drawGUI(bool contentOnly) noexcept
{
    if (!contentOnly) {
        ...
        ImGui::Begin(Language::getText(LanguageID::GUI_TAB_ESP), &windowOpen, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse);
    }
        ...
        for (std::size_t i = 0; i < categories.size(); ++i) {
            ...
            const auto items = [](std::size_t category) noexcept -> std::vector<const char*> {
                ...
                for (std::size_t j = 0; j < items.size(); ++j) {
                    static bool selectedSubItem = false;
