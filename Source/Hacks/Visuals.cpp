...
struct VisualsConfig {
    ...
    struct FootstepESP {
        ColorToggle color{ 0.2f, 0.5f, 1.f, 1.0f };
...
static void from_json(const json& j, VisualsConfig::FootstepESP& ft)
{
    read<value_t::object>(j, "Enabled", ft.color);
...
static void from_json(const json& j, VisualsConfig& v)
{
    ...
    read(j, "Grenade predict", v.nadePredict);
    read(j, "Zoom", v.zoom);
...
static void to_json(json& j, const VisualsConfig::FootstepESP& o, const VisualsConfig::FootstepESP& dummy)
{
    WRITE("Enabled", color);
...
bool Visuals::shouldNadePredict() noexcept
{
    /*
    static auto nadeVar{ interfaces->cvar->findVar("cl_grenadepreview") };

    nadeVar->onChangeCallbacks.size = 0;
    nadeVar->setValue(visualsConfig.nadePredict);
    */
    return visualsConfig.nadePredict;
}
...
ColorToggle Visuals::getMolotovColor() noexcept
{
    return visualsConfig.molotovColor;
}

ColorToggle Visuals::getSmokeColor() noexcept
{
    return visualsConfig.smokeColor;
}
...
void Visuals::removeGrass(csgo::FrameStage stage) noexcept
{
    ...
    constexpr auto getGrassMaterialName = []() noexcept -> const char* {
        switch (fnv::hashRuntime(interfaces->engine->getLevelNameShort())) {
...
{
    if (!visualsConfig.footsteps.color.enabled)
        return;

    const auto entity = interfaces->entityList->getEntity(interfaces->engine->getPlayerForUserID(event.getInt("userid")));

    if (!entity || !localPlayer || entity == localPlayer.get() || entity->isDormant() || !entity->isAlive() || !entity->isOtherEnemy(localPlayer.get()))
        return;

    if (entity->getAbsOrigin() == localPlayer->getAbsOrigin()) // fix for weird bug
        return;
    ...
    */
    ...
    const auto modelIndex = interfaces->modelInfo->getModelIndex("sprites/white.vmt");

    BeamInfo info;

    info.type = BeamEffects::TE_BEAMRINGPOINT;
    info.modelName = "sprites/white.vmt";
    info.modelIndex = modelIndex;
    info.haloIndex = -1;
    info.haloScale = 0.0f;
    info.life = 2.0f;
    info.width = static_cast<float>(visualsConfig.footsteps.footstepBeamThickness);
    info.fadeLength = 0.0f;
    info.amplitude = 0.0f;
    info.red = visualsConfig.footsteps.color.asColor4().color[0] * 255;
    info.green = visualsConfig.footsteps.color.asColor4().color[1] * 255;
    info.blue = visualsConfig.footsteps.color.asColor4().color[2] * 255;
    info.brightness = 255;
    info.speed = 0.0f;
    info.startFrame = 0;
    info.frameRate = 60.0f;
    info.segments = 1;
    info.flags = BeamTypes::FBEAM_FADEOUT;
    info.ringCenter = entity->getAbsOrigin() + Vector(0.0f, 0.0f, 5.0f);
    info.ringStartRadius = 0.0f;
    info.ringEndRadius = static_cast<float>(visualsConfig.footsteps.footstepBeamRadius);
...
void Visuals::doPostProcessing() noexcept
{
    if (!localPlayer)
        return;

    static bool* originalAutoExposureMax = nullptr;
    static bool* originalAutoExposureMim = nullptr;
    static bool* originalCustomBloomScale = nullptr;
    ...
    for (int i = 0; i < 2048; i++)
    {
        ...
        if (originalAutoExposureMax == nullptr)
            originalAutoExposureMax = &ent->useCustomAutoExposureMax();

        if (originalAutoExposureMim == nullptr)
            originalAutoExposureMim = &ent->useCustomAutoExposureMin();

        if (originalCustomBloomScale == nullptr)
            originalCustomBloomScale = &ent->useCustomBloomScale();

        if (!visualsConfig.customPostProcessing.enabled)
        {
            ent->useCustomAutoExposureMax() = originalAutoExposureMax;
            ent->useCustomAutoExposureMin() = originalAutoExposureMim;
            ent->useCustomBloomScale() = originalCustomBloomScale;
            return;
        }
        ...
        if (visualsConfig.disablePostProcessing)
            visualsConfig.disablePostProcessing = false;
...
void Visuals::updateEventListeners(bool forceRemove) noexcept
{
    ...
    public:
        void fireGameEvent(GameEvent* event) override { footstepESP(*event); }
    };

    static FootstepEventListener footstepListener;
    static bool footstepListenerRegistered = false;

    if (visualsConfig.footsteps.color.enabled && !footstepListenerRegistered) {
        interfaces->gameEventManager->addListener(&footstepListener, "player_footstep");
        footstepListenerRegistered = true;
    }
    else if ((!visualsConfig.footsteps.color.enabled || forceRemove) && footstepListenerRegistered) {
...
{
    if (ImGui::MenuItem(Language::getText(LanguageID::GUI_TAB_VISUALS))) {
        windowOpen = true;
        ImGui::SetWindowFocus(Language::getText(LanguageID::GUI_TAB_VISUALS));
        ImGui::SetWindowPos(Language::getText(LanguageID::GUI_TAB_VISUALS), { 100.0f, 100.0f });
    }
}

void Visuals::tabItem() noexcept
{
    if (ImGui::BeginTabItem(Language::getText(LanguageID::GUI_TAB_VISUALS))) {
        drawGUI(true);
        ImGui::EndTabItem();
    }
}
...
void Visuals::drawGUI(bool contentOnly) noexcept
{
    if (!contentOnly) {
        ...
        ImGui::Begin(Language::getText(LanguageID::GUI_TAB_VISUALS), &windowOpen, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize
    ...
    ImGui::Checkbox("Grenade prediction", &visualsConfig.nadePredict);

    ImGui::Checkbox("Custom post-processing", &visualsConfig.customPostProcessing.enabled);
    ImGui::SameLine();
    if (ImGui::Button("...##post-effect"))
        ImGui::OpenPopup("popup_post-effect");

    if (ImGui::BeginPopup("popup_post-effect")) {
    ...
    ImGuiCustom::colorPicker("Molotov Color", visualsConfig.molotovColor);
    ImGuiCustom::colorPicker("Smoke Color", visualsConfig.smokeColor);
    ...
    ImGuiCustom::colorPicker("Footsteps", visualsConfig.footsteps.color);
    ImGui::SliderInt("Thickness", &visualsConfig.footsteps.footstepBeamThickness, 0, 30, "Thickness: %d%%");
    ImGui::SliderInt("Radius", &visualsConfig.footsteps.footstepBeamRadius, 0, 230, "Radius: %d%%");
