...
struct MiscConfig {
    ...
    bool invUnlock{ false };
    bool bypassSvPure{ false };
    bool autoSaveStatTrak{ false };
    bool fakePrime{ false };
    bool bypassInsecure{ false };
...
bool Misc::shouldInvUnlock() noexcept
{
    return miscConfig.invUnlock;
}

bool Misc::shouldAutoSaveStatTrak() noexcept
{
    return miscConfig.autoSaveStatTrak;
}
...
bool Misc::shouldBypassInsecure() noexcept
{
    return miscConfig.bypassInsecure;
}
...
void Misc::recoilCrosshair(ImDrawList* drawList) noexcept
{
    ...
    if (!localPlayerData.shootingAndPistol)
        return;
...
void Misc::bunnyHop(UserCmd* cmd) noexcept
{
    ...
    auto lastJumped = false;
    auto shouldFake = false;
...
void Misc::fakePrime() noexcept
{
    ...
    if (miscConfig.fakePrime && !memory->fakePrime)
        return;
...
void Misc::autoStrafe(UserCmd* cmd, Vector& currentViewAngles) noexcept
{
    ...
    else if (miscConfig.autoStrafe == 2)
    {
        ...
        const auto delta = MathUtils::normalizeYaw(currentViewAngles.y - Helpers::rad2deg(std::atan2(EnginePrediction::getVelocity().y, EnginePrediction::getVelocity().x)));

        cmd->sidemove = delta > 0.f ? -450.f : 450.f;

        currentViewAngles.y = MathUtils::normalizeYaw(currentViewAngles.y - delta);
    }
    else if (miscConfig.autoStrafe == 3)
    {
        if ((localPlayer->flags() & 1) || (EnginePrediction::getFlags() & 1) || localPlayer->moveType() == MoveType::NOCLIP || localPlayer->moveType() == MoveType::LADDER)
            return;
        ...
        if (forwardMove || sideMove)
        {
            ...
            viewAngles.y += turnAngle * 57.295779513082f;
        }
        else if (forwardMove)
            cmd->forwardmove = .0f;
        ...
        orientation.y = MathUtils::normalizeYaw(orientation.y);
        ...
        if (FABSYawDelta <= strafeAngle || FABSYawDelta >= 30.f)
        {
            ...
            MathUtils::vectorAngles(velocity, VelocityAngles);
            ...
            if (velocityAngleYawDelta <= velocityDegree || velocity.length2D() <= 15.f)
            {
                if (-velocityDegree <= velocityAngleYawDelta || velocity.length2D() <= 15.f)
                {
                    ...
                    cmd->sidemove = cl_sidespeed->getFloat() * turnDirectionModifier;
                }
                else
                {
        ...
        MathUtils::vectorAngles(move, moveAngles);
...
void Misc::killfeedChanger(GameEvent& event)
{
    if (!miscConfig.killfeedChanger.enabled)
        return;

    if (const auto localUserId = localPlayer->getUserId(); event.getInt("attacker") != localUserId || event.getInt("userid") == localUserId)
        return;

    if (miscConfig.killfeedChanger.headshot)
        event.setInt("headshot", 1);

    if (miscConfig.killfeedChanger.dominated)
        event.setInt("Dominated", 1);

    if (miscConfig.killfeedChanger.revenge)
        event.setInt("Revenge", 1);

    if (miscConfig.killfeedChanger.penetrated)
        event.setInt("penetrated", 1);

    if (miscConfig.killfeedChanger.noscope)
        event.setInt("noscope", 1);

    if (miscConfig.killfeedChanger.thrusmoke)
        event.setInt("thrusmoke", 1);

    if (miscConfig.killfeedChanger.attackerblind)
...
{
    if (ImGui::MenuItem(Language::getText(LanguageID::GUI_TAB_MISC))) {
        windowOpen = true;
        ImGui::SetWindowFocus(Language::getText(LanguageID::GUI_TAB_MISC));
        ImGui::SetWindowPos(Language::getText(LanguageID::GUI_TAB_MISC), { 100.0f, 100.0f });
    }
}

void Misc::tabItem() noexcept
{
    if (ImGui::BeginTabItem(Language::getText(LanguageID::GUI_TAB_MISC))) {
        drawGUI(true);
        ImGui::EndTabItem();
    }
}
...
void Misc::drawGUI(bool contentOnly) noexcept
{
    if (!contentOnly) {
        ...
        ImGui::Begin(Language::getText(LanguageID::GUI_TAB_MISC), &windowOpen, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoResize
    ...
    ImGui::Combo("Auto strafe", &miscConfig.autoStrafe, "Disable\0Legit\0Direction\0Rage\0");
    ...
    */
    ...
    ImGui::Checkbox("Bypass insecure", &miscConfig.bypassInsecure);
    ImGui::Checkbox("Inventory unlocker", &miscConfig.invUnlock);
    ImGui::Checkbox("Bypass sv pure", &miscConfig.bypassSvPure);
#endif
    ImGui::Checkbox("AutoSaveStatTrakUpdate", &miscConfig.autoSaveStatTrak);
    ...
    ImGui::Checkbox("Killfeed changer", &miscConfig.killfeedChanger.enabled);
    ImGui::SameLine();
    ...
    if (ImGui::BeginPopup("")) {
        ImGui::Checkbox("Headshot", &miscConfig.killfeedChanger.headshot);
        ImGui::Checkbox("Dominated", &miscConfig.killfeedChanger.dominated);
        ImGui::Checkbox("Revenge", &miscConfig.killfeedChanger.revenge);
        ImGui::Checkbox("Penetrated", &miscConfig.killfeedChanger.penetrated);
        ImGui::Checkbox("Noscope", &miscConfig.killfeedChanger.noscope);
        ImGui::Checkbox("Thrusmoke", &miscConfig.killfeedChanger.thrusmoke);
        ImGui::Checkbox("Attackerblind", &miscConfig.killfeedChanger.attackerblind);
        ImGui::EndPopup();
    }
    ImGui::PopID();
...
static void from_json(const json& j, MiscConfig::KillfeedChanger& r)
{
    read(j, "Enabled", r.enabled);
    read(j, "Headshot", r.headshot);
    read(j, "Dominated", r.dominated);
    read(j, "Revenge", r.revenge);
    read(j, "Penetrated", r.penetrated);
    read(j, "NoScope", r.noscope);
    read(j, "Thrusmoke", r.thrusmoke);
    read(j, "Blinded", r.attackerblind);
}
...
static void from_json(const json& j, MiscConfig& m)
{
    ...
    read(j, "Bypass insecure", m.bypassInsecure);
    read(j, "Auto save stattrak", m.autoSaveStatTrak);
    read(j, "Bypass sv pure", m.bypassSvPure);
    read(j, "Inventory Unlocker", m.invUnlock);
...
static void to_json(json& j, const MiscConfig& o)
{
    ...
    WRITE("Bypass insecure", bypassInsecure);
    WRITE("Auto save stattrak", autoSaveStatTrak);
    WRITE("Bypass sv pure", bypassSvPure);
    WRITE("Inventory Unlocker", invUnlock);
