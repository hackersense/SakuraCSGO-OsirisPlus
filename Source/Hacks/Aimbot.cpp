...
void Aimbot::fixMouseDelta(UserCmd* cmd) noexcept
{
    ...
    delta.z = 0.0f;
    const auto sensitivity = interfaces->cvar->findVar("sensitivity");

    if (delta.x != 0.f) {
        const auto pitch = interfaces->cvar->findVar("m_pitch");

        int final_dy = static_cast<int>((delta.x / pitch->getFloat()) / sensitivity->getFloat());
        if (final_dy <= 32767) {
            if (final_dy >= -32768) {
    ...
    if (delta.y != 0.f) {
        const auto yaw = interfaces->cvar->findVar("m_yaw");

        int final_dx = static_cast<int>((delta.y / yaw->getFloat()) / sensitivity->getFloat());
        if (final_dx <= 32767) {
            if (final_dx >= -32768) {
