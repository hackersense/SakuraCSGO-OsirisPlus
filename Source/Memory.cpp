...
static std::uintptr_t findPattern(ModuleInfo moduleInfo, std::string_view pattern) noexcept
{
    ...
    if constexpr (ReportNotFound)
        MessageBoxA(nullptr, ("Failed to find pattern #" + std::to_string(id) + '!').c_str(), "Sakura", MB_OK | MB_ICONWARNING);
#endif
    return 0;
}
...
Memory::Memory() noexcept
{
    ...
    //initSafeModule = reinterpret_cast<decltype(initSafeModule)>(findPattern(CLIENT_DLL, "\x56\x8B\x71\x3C\xB8"));
    ...
    isLoadoutAllowed = findPattern(CLIENT_DLL, "\x84\xC0\x75\x05\xB0\x01\x5F");
    fakePrime = reinterpret_cast<std::uint8_t*>(findPattern<false>(CLIENT_DLL, "??????\x85\xC9\x75\x04\x33\xC0\xEB\x1E"));
    ...
    isSecureServerAllowed = findPattern(ENGINE_DLL, "\x56\x8B\x35????\xFF\xD6\x68????\x8B\xC8\x8B\x10\xFF\x52\x2C\x85\xC0\x75\x2B");
    disableAllowSecuredServers = findPattern(ENGINE_DLL, "\x80\x3D?????\x74\x26\x8B\x0D????");
    ...
    const auto steamAPI = GetModuleHandleW(L"steam_api");

    if (steamAPI)
    {
        SteamClient* steamClient = ((SteamClient*(__cdecl*)(void))GetProcAddress(steamAPI, "SteamClient"))();
        void* steamUser = ((void*(__cdecl*)(void))GetProcAddress(steamAPI, "SteamAPI_GetHSteamUser"))();
        void* steamPipe = ((void* (__cdecl*)(void))GetProcAddress(steamAPI, "SteamAPI_GetHSteamPipe"))();
        steamGameCoordinator = steamClient->getGenericInterface(steamUser, steamPipe, "SteamGameCoordinator001");
    }
    ...
    predictionPlayer = *reinterpret_cast<int**>(findPattern(CLIENT_DLL, "\x89\x35????\xF3\x0F\x10\x48") + 2);
    ...
    soundMessages = relativeToAbsolute<decltype(soundMessages)>(findPattern(ENGINE_DLL, "\x41\x5C\x5D\xE9????\x8B\x48\x08") - 4);
    splitScreen = relativeToAbsolute<decltype(SplitScreen*)>(findPattern(ENGINE_DLL, "\xC6\x05?????\x48\x89\x05????\x0F\x84") + 10);
