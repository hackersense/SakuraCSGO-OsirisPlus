...
inline constexpr float M_PIF = static_cast<float>(M_PI);
...
namespace MathUtils {
	...
	inline void setRandomSeed(int seed) noexcept
	{
		static auto random_seed = reinterpret_cast<void(*)(int)>(GetProcAddress(GetModuleHandleW(L"vstdlib.dll"), "RandomSeed"));

		random_seed(seed);
	}

	inline float randomFloat(float min, float max) noexcept
	{
		static auto random_float = reinterpret_cast<float(*)(float, float)>(GetProcAddress(GetModuleHandleW(L"vstdlib.dll"), "RandomFloat"));
	...
	inline void vectorAngles(const Vector& forward, Vector& angles)
	{
		...
		{
			view[1] = atan2(forward[1], forward[0]) * 180.0f / M_PIF;

			if (view[1] < 0.0f)
				view[1] += 360.0f;

			view[2] = sqrt(forward[0] * forward[0] + forward[1] * forward[1]);
			view[0] = atan2(forward[2], view[2]) * 180.0f / M_PIF;
		}
